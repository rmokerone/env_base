...
const unsigned char PROGMEM Ascii_1[][5]={                // Refer to "Times New Roman" Font Database...
//   Basic Characters
...
const unsigned char PROGMEM oled_32x32[][16]={
...
const unsigned char PROGMEM tmp_char[][32]={
...
0x0C,0x0C,0x0C,0x0C,0x0C,0x04,0x04,0x06,0x02,0x01,0x00,0x00,0x00,0x00,0x00,0x00},/*"℃",0*/
};
...
void oled_clear()
{
        ...
        for (page=0; page<8; page++)
        {
                ...
                SPI_MasterTransmit(0x00); SPI_MasterTransmit(0x10);
                PORTD|=(1<<DC);   //发送数据
                for (i=0; i<128; i++)
...
{
     PORTD&=~(1<<DC); //发送命令
     SPI_MasterTransmit (0xb0+y);
     SPI_MasterTransmit (((x&0xf0)>>4)|0x10);
     SPI_MasterTransmit ((x&0x0f)|0x01);
}
...
void OLED_Display_32(unsigned char x, unsigned char y,char num)
{
     ...
     for (line = 0; line < 4; line ++)
     {
        PORTD |= (1<<DC);
     	...
     	{
		SPI_MasterTransmit (pgm_read_byte(oled_32x32[num * 4 + line]+count));
     	}
	SPI_MasterTransmit (0);
	oled_set_pos (x, y+line+1);
        //switchline (line);
     }
//     SPI_MasterTransmit (0);
}
...
void OLED_Display_tmpchar (unsigned char x, unsigned char y)
{
     ...
     for (line = 0; line < 4; line ++)
     {
        PORTD |= (1<<DC);
     	...
     	{
		SPI_MasterTransmit (pgm_read_byte(tmp_char[line]+count));
     	}
	SPI_MasterTransmit (0);
	oled_set_pos (x, y+line+1);
        //switchline (line);
